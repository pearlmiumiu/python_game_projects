...
# mouseclick handlers that reset UI and conditions whether splash im
#is drawn
def click(pos):
...
def draw(canvas):
    global time, started, rock_group,lives,my_ship,missile_group,score,timer
    ...
    count= group_group_collide(rock_group, missile_group)
    score=score+count
...
# timer handler that s a rock    
def rock_spawner():
    global rock_group,my_ship
    if len(rock_group)>=12:
        return
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    ship_rock_dist=dist(rock_pos, my_ship.pos)
    if ship_rock_dist<90:
        return
...
def group_group_collide(rock_group, missile_group):
    ...
    for rock in set(rock_group):
        ...
        hit_rock=group_collide(missile_group, rock)
        if hit_rock==True:
